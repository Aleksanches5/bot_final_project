...
Отвечай на русском языке. Строго запрещено использовать символы форматирования: **, *, _, __, #, ##, >, `. Пиши только обычным текстом без какого-либо markdown. Не используй markdown символы (**, *, #) — только обычный текст."""
    if knowledge_chunks:
