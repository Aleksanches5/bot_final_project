...
from database.db import (
    add_message, get_history, get_user_ad_data, save_document
)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ─── Определить, есть ли ссылки ─────────────────────────────────────────
    urls = URL_RE.findall(text)
    if urls:
        await _handle_urls(update, context, user_id, urls, text)
        return

    # ─── Обычный вопрос / запрос на анализ ──────────────────────────────────
    await _handle_chat(update, context, user_id, text)


async def _handle_urls(update, context, user_id: int, urls: list[str], original_text: str):
    """Загрузить и сохранить справочные страницы по ссылкам."""
...
async def _handle_chat(update, context, user_id: int, user_text: str):
    """Обработать вопрос через GigaChat с RAG-контекстом."""
    # Показать "печатает..."
    ...
    try:
        ...
        # 8. Отправить ответ (Telegram ограничивает 4096 символов)
        ...
        await update.message.reply_text(
            "❌ Ошибка при обращении к GigaChat. Попробуй снова через несколько секунд.\n"
            f"Детали: {str(e)}"
        )


def _build_ad_data_summary(ad_data: list[dict]) -> str:
    """Собрать краткую сводку по всем загруженным рекламным данным."""
    if not ad_data:
        return ""

    lines = []
    for entry in ad_data[-10:]:  # последние 10 датасетов
        channel = entry.get("channel", "?")
        source = entry.get("source_file", "?")
        data = entry.get("data_json", {})

        # Для Excel (несколько листов) берём первый лист
        ...
        lines.append(f"\n📊 {source} ({channel})")
        lines.append(f"  Строк: {shape[0]}, метрики: {', '.join(columns[:6])}")
